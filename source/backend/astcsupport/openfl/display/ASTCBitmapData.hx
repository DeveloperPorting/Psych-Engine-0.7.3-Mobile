...
import backend.astcsupport.hxastcenc.astc.ASTCFormat;
...
class ASTCBitmapData extends BitmapData {
	...
	public static function loadFromPngFile(url:String, ?astcProperties:ASTCEncodeProperties):ASTCBitmapData {
		var bytes = backend.astcsupport.hxastcenc.astc.ios.AppleASTCEncoder.encodeASTCFromFile(url, astcProperties);
		if (bytes == null) {
			trace("Bytes is null.");
			return null;
		}
		return backend.astcsupport.openfl.display.ASTCBitmapData.fromBytes(bytes);
	}
	#end
}
